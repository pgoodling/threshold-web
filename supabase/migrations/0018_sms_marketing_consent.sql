-- Threshold Salon — separate opt-in for promotional texts.
--
-- Migration 0013 captured consent for booking confirmations, reminders, and
-- replies. That is TRANSACTIONAL consent and it does not cover marketing:
-- promotional texts need prior express written consent under the TCPA, which is
-- a higher bar and must say so explicitly. Someone who ticked "text me about my
-- appointment" has not agreed to receive offers.
--
-- So this is a second, independent column rather than a widening of the first.
-- A client can hold either, both, or neither. The A2P campaign registration is
-- the carrier's permission to send a class of message; THIS is the individual's
-- permission, and both are required before a promotional text may go out.
--
-- Deliberately not backfilled from sms_consent_at. Inferring marketing consent
-- from transactional consent is exactly the thing that makes it invalid.
...
comment on column public.clients.sms_marketing_consent_at is
  'When this client agreed to receive PROMOTIONAL texts. Null = no marketing consent; transactional consent (sms_consent_at) does not substitute.';
comment on column public.clients.sms_marketing_consent_source is
  'Where marketing consent was captured, e.g. booking_form, in_person.';

create index if not exists clients_sms_marketing_consent_idx
  on public.clients (sms_marketing_consent_at)
  where sms_marketing_consent_at is not null;

-- ---------------------------------------------------------------------------
-- create_booking: carry the second tick through as well
-- ---------------------------------------------------------------------------

drop function if exists public.create_booking(uuid, timestamptz, text, text, text, text, text, boolean);
...
begin
  ...
  if v_client_id is null then
    ...
    -- Ticking either box again refreshes that proof. Leaving one unticked never
    -- clears consent already given — only STOP does that.
...
-- ---------------------------------------------------------------------------
-- merge_client: carry marketing consent across a merge too
-- ---------------------------------------------------------------------------
...
begin
  ...
  v_mkt     := greatest(v_survivor.sms_marketing_consent_at, v_loser.sms_marketing_consent_at);
  ...
  update public.clients set
    ...
    email_opt_out      = v_survivor.email_opt_out or v_loser.email_opt_out,
    ...
    sms_marketing_consent_at = v_mkt,
    sms_marketing_consent_source = case
      when v_mkt is null then null
      when v_mkt = v_survivor.sms_marketing_consent_at then v_survivor.sms_marketing_consent_source
      else v_loser.sms_marketing_consent_source
    end,
    notes              = case
