...
export default function CancelPanel({
  ...
  const canCancel = live && hoursAway >= CANCEL_NOTICE_HOURS;
  ...
  async function cancel() {
    ...
    if (!res.ok) {
      ...
      setError(
        json.error === "too_late"
          ? "That's now inside 24 hours — please text or call Evelyn."
  ...
  if (!canCancel) {
    return (
      <div className="rounded-xl border border-foreground/15 bg-white p-5">
        ...
        <p className="mt-2 text-sm text-muted">
          {hoursAway > 0
            ? "It's within 24 hours now, so please text or call Evelyn directly — she'll sort it out with you."
